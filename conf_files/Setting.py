...
class Banner:
    def __init__(self):
        title = "flagger0x1"
        # title = figlet_format("flagger",font="epic",width=300)
        banner = Style.BRIGHT + Fore.RED + title + "\t\t\t\t\tby synfosec\n" + Style.RESET_ALL
        print(banner)

#// POST EXPLOITATION SUCCESS MESSAGE
class SuccessScreen:
    ...
    def connection(self):
        ...
        if(args.server):
            ...
            try:
                ...
                try:
                    p = pwn.remote(args.address,args.port,timeout=10)
                    # p = pwn.listen(args.port)
                    # p.wait_for_connection()
                    p.interactive(prompt=None)
